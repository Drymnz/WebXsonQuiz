package Lengua;

/*
 * Click nbfs://nbhost/SystemFileSystem/Templates/Licenses/license-default.txt to change this license
 * Click nbfs://nbhost/SystemFileSystem/Templates/Classes/Class.java to edit this template
 */
/**
 *
 * @author drymnz
 */
public class LanguageConstants {

    public final static String CREATE_DATA_BASE_USERS = "Se ha creado la base de datos.";
    public final static String DATA_BASE_BLANK = "La base de datos está vacía.";
    public final static String DATA_BASE_NOT_BLANK = "La base de datos no está vacía.";
    public final static String DATA_USER_INCORECT = "Datos de usuario incorrectos.";
    public final static String LOCATION_ON_THE_PORT = "El puerto está ubicado en ";
    public final static String POSSIBLE_IP_SOCKET = "La dirección IP del socket es ";
    public final static String EMPTY_TEXT = "No hay nada en la peticion.";
    public final static String DATABASE_UPDATA_USER = "Se modifico y se actualizo la base de datos de los usuarios.";
    public final static String NOT_DATABASE_UPDATA_USER = "Error al actualizar la base de datos de los usuarios.";
    public final static String APPROVED_REQUEST = "_APROVADO";
    public final static String DENIED_REQUEST = "_DENEGADO";
    public final static String DATABASE_UPDATA_TRIVIA = "Se modifico y se actualizo la base de datos de las trivias.";
    public final static String NOT_DATABASE_UPDATA_TRIVIA = "Error al actualizar la base de datos de las trivias.";
    public final static String OUT_CLIENT_SERVERT = "Salio del servidor.";
}
